...
// this project includes a number of vectorized operations.
// if you wish to make use of these, define the preprocessor symbol DRAGAZO_MATRICES_VECTORSIZE to the desired vector size (e.g. 128 or 256).
// if this is not defined or set to zero then vectorization is not used.
#define DRAGAZO_MATRICES_VECTORSIZE 0

// ----------------------------------------------------------------------------------------------------

#ifdef DRAGAZO_MATRICES_VECTORSIZE
#if DRAGAZO_MATRICES_VECTORSIZE != 0
#include <immintrin.h>
#endif
inline constexpr std::size_t vector_size = DRAGAZO_MATRICES_VECTORSIZE; // size (in bits) of the vector register to use
#else
inline constexpr std::size_t vector_size = 0;
#endif

// ----------------------------------------------------------------------------------------------------

static_assert(CHAR_BIT == 8, "/r/AssholeDesign");

static_assert(vector_size == 0 || (vector_size >= 128 && (vector_size & (vector_size - 1)) == 0), "invalid vector size specified");

template<typename T> inline constexpr bool is_vectorizable_type = std::is_same_v<T, float> || std::is_same_v<T, float>;
...
// for any type T, this acts as an allocator that provides proper alignment and padding for trivialized vector operations
template<typename T>
struct vectorizable_allocator
{
	typedef T value_type;
	static constexpr std::size_t alignment = is_vectorizable_type<T> && vector_size ? std::max(vector_size / 8, alignof(T)) : alignof(T);

	static_assert(std::is_same_v<T, std::decay_t<T>>, "T must not be a non-cv-qualified value type");
	static_assert(alignment != 0 && (alignment & (alignment - 1)) == 0, "alignment is not a power of 2");
	static_assert(alignment >= alignof(T), "alignment is not at least that of T");

	template<typename U> struct rebind { typedef vectorizable_allocator<U> other; };
	...
	[[nodiscard]] T *allocate(std::size_t n)
	{
		...
		{
			std::size_t bytes = n * sizeof(T);       // convert n to a number of bytes
			bytes += (~bytes + 1) & (alignment - 1); // round up to next multiple of alignment

			void *const raw = ::operator new(bytes + sizeof(void*) + alignment - 1); // allocate the raw memory with extra space for bookkeeping and alignment

			void *ret = (char*)raw + sizeof(void*);                            // skip bookkeeping space
			ret = (char*)ret + ((~(std::uintptr_t)ret + 1) & (alignment - 1)); // align ret

			reinterpret_cast<void**>(ret)[-1] = raw; // store bookkeeping
	...
	friend bool operator==(const vectorizable_allocator&, const vectorizable_allocator&) noexcept { return true; }
	friend bool operator!=(const vectorizable_allocator&, const vectorizable_allocator&) noexcept { return false; }
};

// given an allocator type, checks if the allocator is guaranteed to be safely and trivially vectorizable.
template<typename T> struct is_vectorizable_allocator : std::false_type {};
template<typename T> struct is_vectorizable_allocator<vectorizable_allocator<T>> : std::true_type {};

// exception type thrown for matrix operations of incompatible sizes
...
public:
	...
	_unary_expr(const A &_a) noexcept : a(_a) {}
...
public:
	...
	_binary_expr(const A &_a, const B &_b) : a(_a), b(_b) { assert(a.size() == b.size()); }
...
public:
	...
	_vec_scalar_expr(const Vec &v, const Scalar &s) : vec(v), scalar(s) {}
	_vec_scalar_expr(const Vec &v, Scalar &&s) : vec(v), scalar(std::move(s)) {}
...
public: // -- interface -- //
	...
	template<typename E, std::enable_if_t<is_expr<E>::value && std::is_assignable_v<T_cv&, decltype((*(const E*)0)[0])>, int> = 0>
	...
	decltype(auto) move() const { return _unary_expr<_row_view, mover>{ *this }; }
...
template<typename A, typename OP> struct is_expr<_unary_expr<A, OP>> : std::true_type {};
template<typename A, typename B, typename OP> struct is_expr<_binary_expr<A, B, OP>> : std::true_type {};
template<typename Vec, typename Scalar, typename OP, bool left_scalar> struct is_expr<_vec_scalar_expr<Vec, Scalar, OP, left_scalar>> : std::true_type {};

template<typename T_cv> struct is_unseq<_row_view<T_cv>> : std::bool_constant<is_unseq<std::remove_cv_t<T_cv>>::value> {};

template<typename A, typename OP> struct is_unseq<_unary_expr<A, OP>> : std::bool_constant<is_unseq<A>::value> {};
template<typename A, typename B, typename OP> struct is_unseq<_binary_expr<A, B, OP>> : std::bool_constant<is_unseq<A>::value && is_unseq<B>::value> {};
template<typename Vec, typename Scalar, typename OP, bool left_scalar> struct is_unseq<_vec_scalar_expr<Vec, Scalar, OP, left_scalar>> : std::bool_constant<is_unseq<decltype(std::declval<Vec>()[0])>::value && is_unseq<Scalar>::value> {};
...
template<typename T> inline constexpr bool experizable = is_expr<T>::value || is_row_view<T>::value;

// ----------------------------------------------------------------------------------------------------

template<typename A, std::enable_if_t<experizable<A>, int> = 0>
auto operator+(const A &a) noexcept { return _unary_expr<A, noop>{ a }; }
template<typename A, std::enable_if_t<experizable<A>, int> = 0>
auto operator-(const A &a) noexcept { return _unary_expr<A, std::negate<>>{ a }; }

template<typename A, typename B, std::enable_if_t<experizable<A> && experizable<B>, int> = 0>
auto operator+(const A &a, const B &b) noexcept { return _binary_expr<A, B, std::plus<>>{ a, b }; }
template<typename A, typename B, std::enable_if_t<experizable<A> && experizable<B>, int> = 0>
auto operator-(const A &a, const B &b) noexcept { return _binary_expr<A, B, std::minus<>>{ a, b }; }
...
auto operator*(const Vec &vec, Scalar &&scalar) noexcept { return _vec_scalar_expr<Vec, std::decay_t<Scalar>, std::multiplies<>, false>{ vec, std::forward<Scalar>(scalar) }; }
...
auto operator*(Scalar &&scalar, const Vec &vec) noexcept { return _vec_scalar_expr<Vec, std::decay_t<Scalar>, std::multiplies<>, true>{ vec, std::forward<Scalar>(scalar) }; }

template<typename Vec, typename Scalar, std::enable_if_t<experizable<Vec> && !std::is_same_v<decltype((*(const Vec*)0)[0] * (*(const std::decay_t<Scalar>*)0)), void>, int> = 0>
auto operator/(const Vec &vec, Scalar &&scalar) noexcept { return _vec_scalar_expr<Vec, std::decay_t<Scalar>, std::divides<>, false>{ vec, std::forward<Scalar>(scalar) }; }
...
template<typename T, typename Allocator = std::allocator<T>> //typename Allocator = vectorizable_allocator<T>>
class matrix
{
public: // -- requirements -- //
...
private: // -- helper functions -- //
	...
	std::size_t reduce(bool kill_upper, T *det, bool only_det)
	{
		...
		for (std::size_t row = 0, col = 0; row < r; ++row, ++col)
		{
			...
			for (; col < c && self(row, col) == zero; ++col)
			{
...
public: // -- shape utilities -- //
	...
	{
		matrix res(n, n, (T)0);
		const auto f = res.flat();
		for (std::size_t i = 0; i < f.size(); i += n + 1) f[i] = (T)1;
		return res;
	}

	// concatenates other onto the bottom of the matrix
	// throw matrix_size_error if different #cols
	void cat_rows(const matrix &other)
	{
		if (c != other.c) throw matrix_size_error("Cannot vertically concatenate matrices with different #cols");

		data.reserve(data.size() + other.data.size());
		std::copy_n(other.data.begin(), other.data.size(), std::back_inserter(data));
		r += other.r;
	}
	// concatenates other onto the right of the matrix
	// throws matrix_size_error if matrices have different #rows
	void cat_cols(const matrix &other)
	{
		if (r != other.r) throw matrix_size_error("Cannot horizontally concatenate matrices with different #rows");
	...
	// concatenates the matrices vertically
	...
	// concatenates the matrices horizontally
	...
	static matrix cat_cols(matrix &&left, const matrix &right) { left.cat_cols(right); return std::move(left); }
...
public: // -- operations -- //
	...
	matrix getrow(std::size_t row) const
	{
		...
		matrix res(1, c);
		for (std::size_t i = 0; i < c; ++i) res(0, i) = (*this)(row, i);
		return res;
	}
	// extracts column <col> from the matrix (as a column vector)
	matrix getcol(std::size_t col) const
	{
		if (col >= c) throw std::out_of_range("column out of range");

		matrix res(r, 1);
		for (std::size_t i = 0; i < r; ++i) res(i, 0) = (*this)(i, col);
		return res;
	}
...
template<typename T> matrix<T> &operator*=(matrix<T> &m, const T &r)
{
	for (T &v : m.flat()) v *= r;
	return m;
}

template<typename T> matrix<T> operator*(const matrix<T> &m, const T &f) { matrix<T> res = m; res *= f; return res; }
template<typename T> matrix<T> operator*(const T &f, const matrix<T> &m) { matrix<T> res = m; res *= f; return res; }
template<typename T> matrix<T> operator*(matrix<T> &&m, const T &f) { m *= f; return std::move(m); }
template<typename T> matrix<T> operator*(const T &f, matrix<T> &&m) { m *= f; return std::move(m); }
